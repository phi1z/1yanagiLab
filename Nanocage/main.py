{
  ...
  "metadata": {
    "colab": {
      ...
      "authorship_tag": "ABX9TyPSdSHnwxdE4BNvM53XYQFF",
  ...
  "cells": [
    {
      ...
      "source": [
        "<a href=\"https://colab.research.google.com/github/phi1z/1yanagiLab/blob/main/Nanocage/main.py\" target=\"_parent\"><img src=\"https://colab.research.google.com/assets/colab-badge.svg\" alt=\"Open In Colab\"/></a>"
      ]
    },
    {
      "cell_type": "code",
    ...
    },
    {
      ...
      "source": [
        ...
        "\n",
        "$$\\vec{F}_{\\rm AMF}(t) = \\vec{A}\\sin(2 \\pi f t)$$\n",
        "\n",
        "where $\\vec{A}$ (unit $\\rm N$) is the magnitude of AMF and $f$ (unit $\\rm Hz$) is the frequency of AMF.\n",
        "\n",
        "### Viscous Resistance\n",
        "\n",
    ...
    },
    {
      ...
      "source": [
        ...
        "#@markdown Set the label of output file (Ex. `100x8cages_label.csv`)\n",
        "\n",
        ...
        "print(f\"Displavement: {Displacement}\")\n",
        ...
        "file_name = f\"{N}x{n}cages_{Label}.csv\"\n",
    ...
    },
    {
      "cell_type": "code",
      "source": [
        "#@title Select result for DFT (**Run only the first time**) { display-mode: \"form\"}\n",
    ...
    },
    {
      ...
      "source": [
        ...
        "# @markdown $i$-th nanocages list that you want to see the graph (**0 means all**)\n",
        "i_list = 0 # @param {\"type\":\"integer\"}\n",
        "if i_list == 0:\n",
        "  i_list = None\n",
        "else:\n",
        "  i_list = [i_list]\n",
        ...
        "# @markdown $j$-th nanocages list that you want to see the peak (**0 means all**)\n",
        "j_list = 0 # @param {\"type\":\"integer\"}\n",
        "if j_list == 0:\n",
        "  j_list = None\n",
        "else:\n",
        "  j_list = [j_list]\n",
        ...
        "  return Y, Y_err\n",
        "\n",
        "def plot_peak(df, flim=25, vec=\"x\", i=0, plot=False):\n",
        "  n_lim = (df.shape[0] * flim) // 100\n",
        "  if i_list != None:\n",
        ...
        "  color_list = [cmap(i/(len(n_list)-1)) for i in n_list]\n",
        "  X = df[\"freq\"]\n",
        "  X = X[1:n_lim]\n",
        ...
        "    if plot:\n",
        "      plt.plot(X[peak+1], lim_y[1] * 1.5**(i+1), color=color_list[i], marker=10, ms=10)\n",
        "  print(out_str[:-2] + \" MHz\")\n",
        "\n",
        "def plot_stat_dft(df, vec=\"x\", se=None, flim=25, i_list=None):\n",
        "  n = (df.shape[1]-1) // 6 - 1\n",
        "  if i_list != None:\n",
        "    n_list = [int(j - 1) for j in i_list]\n",
        "  else:\n",
        ...
        "    plt.fill_between(X, Y_under, Y_over, color=color_list[i], alpha=0.3)\n",
        "    plt.plot(X, Y, \"-\", label=f\"{i+1}_{vec}_ave.\", color=color_list[i])\n",
        "\n",
        ...
        "  s_list = range(n)\n",
        "for i in s_list:\n",
        "  plot_peak(df, flim=freq_range, vec=direction, i=i, plot=Plot_peak)\n",
